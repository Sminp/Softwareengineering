...
class StoryMode(UNOGame):
    def __init__(self):
        ...
        self.storywin = [self.settings['achievement']['storya_win'],self.settings['achievement']['storyb_win'],self.settings['achievement']['storyc_win'],self.settings['achievement']['storyd_win']]
...
class YesNoStory(YesNo):
    def __init__(self, player_num, difficulty, class_name):
        if difficulty == 2:
            self.info = ["A 지역을 선택하시겠습니까?", "첫 분배시 컴퓨터에게 기술 카드를", "50% 더 높은 확률로 지급합니다.", "컴퓨터 플레이어가 콤보를 사용합니다.","인원수 : 2"]
        elif difficulty == 3:
            self.info = ["B 지역을 선택하시겠습니까?", "첫 카드를 제외하고 모든 카드를 같은 ", "수만큼 모든 플레이어들에게 분배합니다.", "인원수 : 4"]
        elif difficulty == 4:
            self.info = ["C 지역을 선택하시겠습니까?", "매 5턴마다 낼 수 있는 카드의", "색상이 무작위로 변경됩니다.","인원수 : 3"]
        else:
            self.info = ["D 지역을 선택하시겠습니까?", "현재 카드의 배수나 약수의 값을 ", "가지는 카드만 낼 수 있습니다.","인원수 : 3"]
        ...
        for i in range(1, len(self.info)):
            self.info_list.append(rf.TextRect(self.screen, self.info[i], 20, c.BLACK))
    ...
    def object_show(self):
        ...
        for i in range(len(self.info_list)):
            self.info_list[i].show((self.settings['screen'][0] / 2, self.settings['screen'][1] / 3 + 40 * i))
